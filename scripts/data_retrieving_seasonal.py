# This script retrieves seasonal climate data for Corn Belt counties from TerraClimate.
...
# TerraClimate provides high-resolution climate data, and the script focuses on the growing season (April to October) to capture relevant conditions for corn growth.
...
# It focuses on the growing season (April to October) and aggregates data accordingly.

            data = data.sel(time=data["time"].dt.month.isin([4,5,6,7,8,9,10]))
...
#centroids are calculated for each county to determine the representative latitude and longitude for data retrieval from TerraClimate.
    ...
    climate["tmax"] /= 10
    climate["tmin"] /= 10
# The output is a CSV file with one row per county-year, containing the aggregated climate variables.

    # save
    output_file = oj(DATA_PATH, "cornbelt_climate_seasonal_1990_2024.csv")
